...
#ifdef __USE_GACT
static enum at_response_type scanner_qgact(const char *line, size_t len, void *arg)
{
    (void) len;
    (void) arg;
    ...
    int state = 0;
    if (sscanf(line, "+QGACT: 1,1,%d", &state) == 1) {
...
static int bc26_pdp_open(struct cellular *modem, const char *apn)
{
    ...
    at_set_command_scanner(modem->at, scanner_qgact);
    const char *response = at_command(modem->at, "AT+QGACT=1,1,\"%s\"", apn);
    int state = 0;
    const char *rsp = strstr(response, "+QGACT: 1,1,");
    if (sscanf(rsp, "+QGACT: 1,1,%d", &state) == 1 && state == 1) {
        return 0;
    }
#else
    struct cellular_bc26 *priv = (struct cellular_bc26 *) modem;

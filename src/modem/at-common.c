...
int cellular_op_iccid(struct cellular *modem, char *buf, size_t len)
{
    ...
    if(len > CELLULAR_ICCID_LENGTH && sscanf(response, "+CCID:%21s", buf) == 1) {
...
int cellular_op_sms(struct cellular *modem, char* num, char* msg, size_t len)
{
    ...
    at_set_timeout(modem->at, AT_TIMEOUT_SHORT);
    // SMS Center
    int ret = at_config(modem->at, "CSCA", strncmp(modem->apn, "TM", strlen("TM")) ? "\"+8613010811500\"" : "\"+447797704000\"", 30);
    if(ret < 0) {
        return ret;
    }
    // Text mode
